...
def linear_MLP(name, x, downsample_factor=4, out_final=0, trainable=True, use_grl=False):
    ...
    with tf.variable_scope(name):
        ...
        with tf.variable_scope(name):
            ...
            n_l_d = int(np.log2(int(x.get_shape()[1])) / 3)
            for i in range(0, n_l_d-1):
                x = tf.layers.dense(x, int(int(x.get_shape()[1])/8), name='fully_conn_'+str(i))

            x = tf.layers.dense(x, out_final, name='fully_conn_l')
