...
def parse_tfrecord_tf(record, att_name):
    ...
    return img_mri, img_pet,  tf.cast(att, tf.float32) , features['id']# att
...
def main():
    ...
    train_iterator, test_iterator,_ = \
             get_data(sess, tf_file_name,2, 1, 1, 1, 1, 1, 1,att=att)


    imgs_mri, imgs_pet, ys, ids= train_iterator.get_next()

    IMG_ms, IMG_ps, ys_, ids_ = [],[],[],[]
    for i in range(726):
        ...
        IMG_ps.append(IMG_p)
        #ys_.append(y)
        ids_.append(str(id) + ',' + str(y))

    x_ = np.concatenate(IMG_ps, axis=0)
    # x_ms = np.concatenate(IMG_ms, axis=0)
    # np.save('../brain3D_sample_m', IMG_m)
    np.save('../train_p', x_)

    with open( '../ordered_train_values.txt', 'w') as f:
        for item in ids_:
            f.write("%s\n" % item)
