...
def parse_tfrecord_tf(record):
    ...
    return img_mri, img_pet, tf.cast(features[att], tf.float32), features['id'] #features['adas']#, features['age'],features['cdr'],tf.cast(features['apoe'], tf.float32), tf.cast(features['dxbl'], tf.float32),tf.cast(features['gender'], tf.float32), tf.cast(features['mmse'], tf.float32), tf.cast(features['ravlt'], tf.float32)
...
def main():
    ...
    train_iterator, test_iterator,_ = \
             get_data(sess, tf_file_name,2, 1, 1, 1, 4, 80, 4)
    ...
    # np.save('../brain3D_sample_label_' + att +'_id', id)

    from PIL import Image
    a = np.expand_dims((IMG_m[1][20, :, :, 0]) * 255, axis=2)
    im = np.concatenate((a, a, a), axis=2)
    ...
    img.show()
    img = Image.fromarray((IMG_m[78][20, :, :, 0]) * 255)
    img.show()
    ...
    img.show()
    img = Image.fromarray((IMG_p[78][20, :, :, 0]) * 255)
    img.show()
