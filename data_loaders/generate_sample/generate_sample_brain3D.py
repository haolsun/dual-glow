...
def parse_tfrecord_tf(record):
    ...
    return img_mri, img_pet, tf.cast(features[att], tf.float32) #features['adas']#, features['age'],features['cdr'],tf.cast(features['apoe'], tf.float32), tf.cast(features['dxbl'], tf.float32),tf.cast(features['gender'], tf.float32), tf.cast(features['mmse'], tf.float32), tf.cast(features['ravlt'], tf.float32)
