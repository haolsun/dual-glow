...
def parse_tfrecord_tf(record):
    ...
    return img_mri, img_pet, tf.cast(features[att], tf.float32), features['id']#tf.cast(features[att], tf.float32) #features['adas']#, features['age'],features['cdr'],tf.cast(features['apoe'], tf.float32), tf.cast(features['dxbl'], tf.float32),tf.cast(features['gender'], tf.float32), tf.cast(features['mmse'], tf.float32), tf.cast(features['ravlt'], tf.float32)
...
def make_batch(sess, itr, itr_batch_size, required_batch_size):
    ...
    # assert rb % ib == 0
    k = int(np.ceil(rb / ib))
    xs_mri, xs_pet, ys, ids = [], [], [], []
    data = itr.get_next()
    for i in range(k):
        x_mri, x_pet, y, id = sess.run(data)
        xs_mri.append(x_mri)
        xs_pet.append(x_pet)
        ys.append(y)
        ids.append(id)
    x_mri, x_pet, y, id = np.concatenate(xs_mri)[:rb], np.concatenate(xs_pet)[:rb], np.concatenate(ys)[:rb], np.concatenate(ids)[:rb]
    return {'x_mri': x_mri, 'x_pet': x_pet, 'y': y, 'id': id}
...
def main():
    ...
    train_iterator, test_iterator, init_ = \
             get_data(sess, tf_file_name,2, 1, 1, 1, 4, 36, 4)

    # sess.run(tf.global_variables_initializer())

    imgs_mri, imgs_pet, ys, ids= test_iterator.get_next()

    IMG_m, IMG_p, y, ID = sess.run([imgs_mri, imgs_pet, ys, ids])


    from PIL import Image
    img = Image.fromarray((IMG_m[1][20, :, :, 0]) * 255)
    img.show()
    img = Image.fromarray((IMG_m[10][20, :, :, 0]) * 255)
    img.show()
    img = Image.fromarray((IMG_m[20][20, :, :, 0]) * 255)
    img.show()
    img = Image.fromarray((IMG_m[35][20, :, :, 0]) * 255)
    img.show()

    img = Image.fromarray((IMG_p[1][20, :, :, 0]) * 255)
    img.show()
    img = Image.fromarray((IMG_p[10][20, :, :, 0]) * 255)
    img.show()
    img = Image.fromarray((IMG_p[20][20, :, :, 0]) * 255)
    img.show()
    img = Image.fromarray((IMG_p[35][20, :, :, 0]) * 255)
    img.show()

    np.save('../brain3D_sample_m', IMG_m)
    np.save('../brain3D_sample_p', IMG_p)

    ################### change ########################
    print(att)
    np.save('../brain3D_sample_label_' + att, y)
